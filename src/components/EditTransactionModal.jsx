...
import { X, CheckCircle, AlertCircle, Loader2, Edit3 } from 'lucide-react';
...
export default function EditTransactionModal({ isOpen, onClose, transaction, onTransactionUpdated, selectedCurrency = 'USD' }) {
  ...
  useEffect(() => {
    if (transaction) {
      ...
      const dateVal = transaction.date ? new Date(transaction.date).toISOString().split('T')[0] : '';
      setDate(dateVal);
    }
  }, [transaction]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setStatusMessage({ type: 'error', text: err.message || 'Error updating transaction' });
    } finally {
      ...
      >
        <div className="flex items-center justify-between pb-4 border-b border-slate-800">
          <div className="flex items-center gap-2">
            <div className="p-2 rounded-xl bg-slate-900 border border-[#74FFAC]/30 text-[#74FFAC]">
              <Edit3 className="w-4 h-4" />
            </div>
            <div>
              <h2 className="text-lg font-bold text-slate-100">Edit Transaction</h2>
              <p className="text-xs text-slate-400">Modify transaction details</p>
            </div>
          </div>
          <button
            type="button"
        ...
        <form onSubmit={handleSubmit} className="mt-4 space-y-4">
          <div>
            <label htmlFor="edit-amount-input" className="block text-xs font-medium text-slate-300 mb-1.5">
          ...
          <div>
            <label htmlFor="edit-merchant-input" className="block text-xs font-medium text-slate-300 mb-1.5">
              Merchant / Store
            </label>
            <input
              id="edit-merchant-input"
              type="text"
              required
          ...
          <div>
            <label htmlFor="edit-category-select" className="block text-xs font-medium text-slate-300 mb-1.5">
              Category
            </label>
            <select
          ...
          <div>
            <label htmlFor="edit-date-input" className="block text-xs font-medium text-slate-300 mb-1.5">
              Date
            </label>
            <input
              id="edit-date-input"
